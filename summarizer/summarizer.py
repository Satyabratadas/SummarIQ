...
#         small_chunks = self.split_into_chunks(section_text, max_tokens=120)
#         partial_summaries = []
...
class Summarizer:
    def __init__(self, model_path):
        print(f"[INFO] Loading T5 model from: {model_path}")
    ...
    # T5 TEXT GENERATION
    # ------------------------------------------------
    def t5_generate(self, prompt, max_len=180):
    ...
    # SUMMARY FUNCTIONS
    # ------------------------------------------------
    def summarize_abstract(self, cleaned_paper):
        abs_text = cleaned_paper["abstract"]

        prompt = (
            "Write a clear academic summary of the following abstract. "
            "Begin with 'This paper investigates' or similar. "
            "Explain the problem, method, results, significance.\n\n"
            "Abstract:\n" + abs_text + "\n\nSummary:"
        )
    ...
    def build_section_prompt(self, section_name, section_text):
        return f"""
Summarize the following academic section in 3–4 clear sentences.
...
"""
    ...
    def summarize_section(self, section):
        prompt = self.build_section_prompt(section["name"], section["content"])
